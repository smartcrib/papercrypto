...
class PyText2Pdf(object):
    ...
    def convert(self):
        ...
        if self._lines==0:
            self._lines = (self._pageHt - 32)/self._vertSpace
        if self._lines < 1:
            self._lines=1
    ...
    def startpage(self):
        ...
        ws(buf)
        buf = "".join(("1 0 0 1 50 ", str(self._pageHt - 20), " Tm\n"))
        ws(buf)
                    ...
                    while charNo < self._cols:
                        ...
                        if ord(ch) >= 32 and ord(ch) <= 127 and ch!='!':
                            ...
                            if ch == '!':
                              ...
                              ws(buf)
                              if LARGE==1:
                                buf = "".join(("1 0 0 1 ",str(50+3.5*charNo+2)," ", str(self._pageHt - 20-(lineNo-1)*self._vertSpace), " Tm\n"))
                                charNo-=1 
                              else:
                                buf = "".join(("1 0 0 1 50 ", str(self._pageHt - 20-(lineNo-1)*self._vertSpace), " Tm\n"))
                                charNo=1;
                              ws(buf)
                              buf = "7 TL\n"
                              ws(buf)
...
def main():
    ...
    pdfclass=PyText2Pdf()
    pdfclass.parse_args("/tmp/34AC95D48A4EF7AD6FD10A609BEDC355DAAD215CAACDF0CC51D83090E5859704")
    pdfclass.convert()
